...
public:
	explicit SPHParticles(std::vector<float3>& p)
		:Particles(p),
		pressure(p.size()),
		density(p.size()),
		mass(p.size()),
	...
	float* getTemperaturePtr() const
	{
		return temperature.addr();
	}
	const DArray<float>& getTemperature() const
	{
		return temperature;
	}
