	...
	bufferInt(max(totalSize(), cellSize.x* cellSize.y* cellSize.z + 1))
{
	...
	thrust::fill(thrust::device, _fluids->getTemperaturePtr(), _fluids->getTemperaturePtr() + (_fluids->size() / 2), 99);
	thrust::fill(thrust::device, _fluids->getTemperaturePtr() + (_fluids->size() / 2), _fluids->getTemperaturePtr() + _fluids->size(), 1);
