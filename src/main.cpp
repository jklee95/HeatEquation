...
#include "DFSPHSolver.h"1
#include "PBDSolver.h"
#include "SPHSystem.h"

 //vbo and GL variables
cudaGraphicsResource_t particlesVBO_CUDA;
cudaGraphicsResource_t particlesColorVBO_CUDA;
...
char  vert_shader_path[256] = "particles.vert";
char  frag_shader_path[256] = "particles.frag";
// view variables
...
void initSPHSystem(const int solver = fluid_solver::SPH) {
	// initiate fluid particles // 20736
	...
	// initiate boundary particles // 14408
	pos.clear();
...
void createVBO(cudaGraphicsResource_t* cudaResourceBuf,GLuint* vbo, const unsigned int length) {
	// create buffer objec;
	glGenBuffers(1, vbo);
	...
	CUDA_CALL(cudaGraphicsGLRegisterBuffer(cudaResourceBuf, *vbo, cudaGraphicsMapFlagsWriteDiscard));
}

void deleteVBO(cudaGraphicsResource_t* cudaResourceBuf,GLuint* vbo) 
{

	glBindBuffer(1, *vbo);
	cudaGraphicsUnregisterResource(*cudaResourceBuf);
...
void onClose(void) 
{
	deleteVBO(&particlesVBO_CUDA,&particlesVBO);
	deleteVBO(&particlesVBO_CUDA,&particlesColorVBO);
	pSystem = nullptr;
...
void initGL(void) {
	// create VBOs
	createVBO(&particlesVBO_CUDA,&particlesVBO, sizeof(float3) * pSystem->size());
	createVBO(&particlesColorVBO_CUDA,&particlesColorVBO, sizeof(float3) * pSystem->size());
	...
	ShaderUtility::attachAndLinkProgram(m_particles_program, ShaderUtility::loadShaders(vert_shader_path, frag_shader_path));
	return;
}
...
void renderParticles(void) {
	...
	cudaGraphicsMapResources(1, &particlesVBO_CUDA,0);
	cudaGraphicsMapResources(1, &particlesColorVBO_CUDA, 0);


	CUDA_CALL(cudaGraphicsResourceGetMappedPointer((void**)&dptr, &bytes, particlesVBO_CUDA));
	CUDA_CALL(cudaGraphicsResourceGetMappedPointer((void**)&cptr, &bytes, particlesColorVBO_CUDA));
	...
	CUDA_CALL(cudaGraphicsUnmapResources(1, &particlesVBO_CUDA, 0));
	CUDA_CALL(cudaGraphicsUnmapResources(1, &particlesColorVBO_CUDA, 0));
